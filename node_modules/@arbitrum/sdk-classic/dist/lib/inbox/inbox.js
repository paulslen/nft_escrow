...
class InboxTools {
    constructor(l1Signer, l2Network) {
        ...
        if (!this.l1Network)
            throw new errors_1.ArbTsError(`L1Network not found for chain id: ${l2Network.partnerChainID}.`);
    }
    /**
        ...
        const multicallInput = [
            {
                ...
                encoder: () => sequencerInbox.interface.encodeFunctionData('maxDelayBlocks'),
                decoder: (returnData) => sequencerInbox.interface.decodeFunctionResult('maxDelayBlocks', returnData)[0],
            },
            {
                targetAddr: sequencerInbox.address,
                encoder: () => sequencerInbox.interface.encodeFunctionData('maxDelaySeconds'),
                decoder: (returnData) => sequencerInbox.interface.decodeFunctionResult('maxDelaySeconds', returnData)[0],
            },
            multicall.getBlockNumberInput(),
            multicall.getCurrentBlockTimestampInput(),
        ];
        const [maxDelayBlocks, maxDelaySeconds, currentBlockNumber, currentBlockTimestamp,] = await multicall.multiCall(multicallInput, true);
        const firstEligibleBlockNumber = currentBlockNumber.toNumber() - maxDelayBlocks.toNumber();
        const firstEligibleTimestamp = currentBlockTimestamp.toNumber() - maxDelaySeconds.toNumber();
    ...
    async getEventsAndIncreaseRange(bridge, searchRangeBlocks, maxSearchRangeBlocks, rangeMultiplier) {
        ...
        const events = await eFetcher.getEvents(bridge.address, Bridge__factory_1.Bridge__factory, b => b.filters.MessageDelivered(), {
    ...
    async forceInclude(messageDeliveredEvent, overrides) {
        ...
        const transactionReceipt = await this.l1Provider.getTransactionReceipt(eventInfo.transactionHash);
        return await sequencerInbox.functions.forceInclusion(eventInfo.event.messageIndex.add(1), eventInfo.event.kind, [eventInfo.blockNumber, block.timestamp], eventInfo.event.messageIndex, transactionReceipt.effectiveGasPrice, eventInfo.event.sender, eventInfo.event.messageDataHash, eventInfo.delayedAcc, 
