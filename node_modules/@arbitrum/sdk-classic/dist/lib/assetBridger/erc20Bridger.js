...
class Erc20Bridger extends assetBridger_1.AssetBridger {
    ...
    async getDepositParams(params) {
        ...
        const estimateGasCallValue = (await this.isWethGateway(l1GatewayAddress, l1Signer.provider))
            ? amount
            : constants_1.Zero;
    ...
    async depositTxOrGas(params, estimate) {
        ...
        const data = abi_1.defaultAbiCoder.encode(['uint256', 'bytes'], [depositParams.maxSubmissionCost, '0x']);
        ...
        return await (estimate
            ...
            : l1GatewayRouter.functions).outboundTransfer(depositParams.erc20L1Address, depositParams.destinationAddress, depositParams.amount, depositParams.maxGas, depositParams.maxGasPrice, data, Object.assign(Object.assign({}, (params.overrides || {})), { value: depositParams.l1CallValue }));
    }
    /**
    ...
    async withdrawTxOrGas(params, estimate) {
        ...
        return (estimate ? l2GatewayRouter.estimateGas : l2GatewayRouter.functions)['outboundTransfer(address,address,uint256,bytes)'](params.erc20l1Address, to, params.amount, '0x', params.overrides || {});
    }
    /**
...
Erc20Bridger.MIN_CUSTOM_DEPOSIT_MAXGAS = ethers_1.BigNumber.from(275000);
/**
...
class AdminErc20Bridger extends Erc20Bridger {
    ...
    async registerCustomToken(l1TokenAddress, l2TokenAddress, l1Signer, l2Provider) {
        ...
        if (l1AddressFromL2 !== l1TokenAddress) {
            throw new errors_1.ArbTsError(`L2 token does not have l1 address set. Set address: ${l1AddressFromL2}, expected address: ${l1TokenAddress}.`);
        }
        ...
        const setTokenEstimates = await gasPriceEstimator.estimateMessage(this.l2Network.tokenBridge.l1CustomGateway, 
        // l1SenderAddress,
        this.l2Network.tokenBridge.l2CustomGateway, l2SetTokenCallData, constants_1.Zero);
        // 2. setGateway
        ...
        const setGatwayEstimates = await gasPriceEstimator.estimateMessage(this.l2Network.tokenBridge.l1GatewayRouter, this.l2Network.tokenBridge.l2GatewayRouter, l2SetGatewaysCallData, constants_1.Zero);
        // now execute the registration
        const customRegistrationTx = await l1Token.registerTokenOnL2(l2TokenAddress, setTokenEstimates.maxSubmissionPriceBid, setGatwayEstimates.maxSubmissionPriceBid, setTokenEstimates.maxGasBid, setGatwayEstimates.maxGasBid, setGatwayEstimates.maxGasPriceBid, setTokenEstimates.totalDepositValue, setGatwayEstimates.totalDepositValue, l1SenderAddress, {
            value: setTokenEstimates.totalDepositValue.add(setGatwayEstimates.totalDepositValue),
        });
    ...
    async getL1GatewaySetEvents(l1Provider, filter, customNetworkL1GatewayRouter) {
        if (this.l2Network.isCustom && !customNetworkL1GatewayRouter) {
            throw new Error('Must supply customNetworkL1GatewayRouter for custom network ');
        }
        await this.checkL1Network(l1Provider);
        const l1GatewayRouterAddress = customNetworkL1GatewayRouter || this.l2Network.tokenBridge.l1GatewayRouter;
    ...
    async getL2GatewaySetEvents(l2Provider, filter, customNetworkL2GatewayRouter) {
        if (this.l2Network.isCustom && !customNetworkL2GatewayRouter) {
            throw new Error('Must supply customNetworkL2GatewayRouter for custom network ');
        }
        await this.checkL2Network(l2Provider);
    ...
    async setGateways(l1Signer, l2Provider, tokenGateways, maxGas = ethers_1.BigNumber.from(0)) {
        ...
        const l1GatewayRouter = L1GatewayRouter__factory_1.L1GatewayRouter__factory.connect(this.l2Network.tokenBridge.l1GatewayRouter, l1Signer.provider);
        const res = await l1GatewayRouter.functions.setGateways(tokenGateways.map(tG => tG.tokenAddr), tokenGateways.map(tG => tG.gatewayAddr), maxGas, l2GasPrice, submissionPrice, {
