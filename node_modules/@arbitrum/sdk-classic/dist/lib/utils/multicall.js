...
class MultiCaller {
    ...
    static async fromProvider(provider) {
        ...
        if (!network) {
            throw new errors_1.ArbTsError(`Unexpected network id: ${chainId}. Ensure that chain ${chainId} has been added as a network.`);
        }
        let multiCallAddr;
        if ((0, networks_1.isL1Network)(network)) {
            const firstL2 = networks_1.l2Networks[network.partnerChainIDs[0]];
            if (!firstL2)
                throw new errors_1.ArbTsError(`No partner chain found l1 network: ${network.chainID} : partner chain ids ${network.partnerChainIDs}`);
        ...
        for (const t of erc20Addresses) {
            ...
            if (defaultedOptions.name) {
                input.push({
                    ...
                    decoder: (returnData) => erc20Iface.decodeFunctionResult('name', returnData)[0],
                });
            }
            if (defaultedOptions.symbol) {
                input.push({
                    ...
                    decoder: (returnData) => erc20Iface.decodeFunctionResult('symbol', returnData)[0],
                });
            }
        }
